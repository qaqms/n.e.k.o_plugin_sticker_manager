...
class LanlanResolver:
    ...
    async def _from_bus(self) -> str:
        ...
        for namespace in (getattr(bus, "conversations", None), getattr(bus, "memory", None)):
            getter = getattr(namespace, "get", None)
            if not callable(getter):
                continue
            try:
                raw: Any = getter(max_count=_SCAN_RECORDS, limit=_SCAN_RECORDS, bucket_id="default")
                if hasattr(raw, "__await__"):
                    raw = await raw
            except Exception:  # noqa: BLE001 - 一次读失败只丢这个命名空间
                self._log("lanlan: bus read failed")
                continue
