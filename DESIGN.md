...
- 表情墙（v0.9.0 轮 G-2，纯视图轮，主人对 v0.8.0 卡片的“表单感”反馈而做）：格子只露图
  （6 列方块；左上角极小勾选框与禁用压暗角标是唯二的例外），全部属性与动作收进点击
  弹出的详情弹窗（看做分离：弹窗=大图+全属性+四动作，「编辑」再开第二层表单）；
  预览改 IntersectionObserver 视口懒加载（提前 240px）+ 全局并发尺限同时 2 张
  （分段串行拉图，几百格同跑会踩子进程；无观察器环境直接排队，宁多拉不漏图）
