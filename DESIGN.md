...
- **交接（2026-09-17，J 系列进行中）**——下一个接手的人/AI 先读这里：
  - **J-1 已完工（v0.11.0，提交 `c694f3d` 已推 origin）**：三层「区→分类→图」落地，
    她只感知激活区（陷阱 23）；旧库 load 即迁入默认区「自制区」。实机验收（0.11.0 包导入 +
    区操作四步）还没跑过——接手先问主人验收结果。
  - **J-2 待开工（官方区内置，主人拍板 P2A 完全内置 + P3 只播种一次+恢复按钮）**：
    **第 0 步是 spike，不通过就全案转 B（配方包放固定路径）**：① 造 ~86MB 含 gif 假包，
    插件中心实导一次验封套链路；② ~250KiB gif 实机发一张验内联通道。
    官方素材 = `F:\绘画\素材\素材\GIF`（190 张 / 86MB / 全单张<8MiB / 中位 461KiB / 最大 960KiB）。
    **硬雷（甲方案已拍板）**：gif 必须内联（images.upload 压平动图，陷阱 1），内联尺 256KiB 只过 40/190——
    所以官方包在 sticker_pack_lab 做 **gifsicle 压图工序（每张 ≤250KiB 硬尺，降尺寸/减帧/收色板）**，
    压完全量收录。播种：首启把内置官方包收进名为「官方」的区（标 builtin），只播一次；
    官方区不存在时面板 tab 栏尾出「恢复官方收藏」；新装默认激活官方区；拆官方区同样 3 秒闸。
    payload 带 86MB 后 release_gate 的复制探针会变慢（预计 +数十秒），属已接受的代价。
