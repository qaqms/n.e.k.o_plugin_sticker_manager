...
## 0.4.0
v0.4.0「表情是她挑的，不是系统拍板」——轮 C：按情境选图（学习 astrbot 的选图纪律，
机制见 docs/astrbot-study.md）：

- **`resolve_send_target` 判定器（core）**：query 选图不再"发匹配第一张"——
  最优分严格唯一→直发；头部并列→回 top-K 候选清单（`multi_candidates`，
  上限 5 与 astrbot top_k 同量级）让她拿 id 发第二刀。**不替她拍板并列**。
...
- **注入文案升级（学 astrbot prompt head 的"使用规则"段）**：中段新增——
